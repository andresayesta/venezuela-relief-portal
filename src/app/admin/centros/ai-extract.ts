...
const ExtractedCenterSchema = z.object({
  ...
  state: z
    .enum(VENEZUELAN_STATES)
    .nullable()
    .describe('Venezuelan state where the center is located.'),
...
export async function extractCenterFromImage(imageUrl: string): Promise<ExtractResult> {
  ...
  try {
    const { output: extracted } = await generateText({
      ...
      instructions: [
        'You extract structured info about a Venezuelan earthquake-relief collection center (centro de acopio) from a photo or screenshot.',
        ...
        'Normalize the state to one of the canonical Venezuelan states.',
        ...
        'For event_date, ONLY return a YYYY-MM-DD date if the image explicitly states a date for a one-time event (e.g. "30 de junio", "sábado 28/06/2026"). Use null for ongoing centers and relative phrases like "este sábado" or "mañana" — you cannot resolve those without knowing today.',
      ].join(' '),
      messages: [
        {
          role: 'user',
